...
# =========================================================
# 청년 인구(만 19~34세) 최다 동네 분석
# =========================================================
st.subheader("🧑‍🎓 청년 인구(만 19~34세) 최다 동네 Top N")
st.caption("읍면동 단위에서 만 19~34세 인구(계 기준)를 집계하여, 청년 인구가 가장 많은 동네를 인구수 또는 전체 인구 대비 비율 기준으로 보여줍니다.")

YOUTH_MIN_AGE, YOUTH_MAX_AGE = 19, 34

# 만 19~34세 '계' 연령 컬럼 추출
youth_age_cols = [
    c for c in age_cols
    if c[1] == "계" and YOUTH_MIN_AGE <= c[2] <= YOUTH_MAX_AGE
]
youth_col_names = [c[0] for c in youth_age_cols]

col1, col2, col3 = st.columns([1.2, 1, 1.5])

with col1:
    youth_metric = st.radio(
        "정렬 기준",
        options=["청년 인구수", "청년 인구 비율(%)"],
        horizontal=True,
    )

with col2:
    top_n = st.slider("표시할 동네 수 (Top N)", min_value=5, max_value=30, value=10, step=1)

sido_options = sorted(
    df.loc[df["단위"] == "시도", "지역명"].unique().tolist()
)
with col3:
...
if not youth_col_names:
    st.warning("만 19~34세 연령 컬럼을 찾지 못했습니다. 데이터 컬럼명을 확인해주세요.")
else:
    dong_df = df[df["단위"] == "읍면동"].copy()

    if sido_filter:
        # 지역명 맨 앞 시도명으로 필터링 (예: "서울특별시 종로구 청운효자동" -> "서울특별시")
        dong_df = dong_df[dong_df["지역명"].str.split().str[0].isin(
            [s.split()[0] for s in sido_filter]
        )]

    total_pop_col = next((c for c in df.columns if c.endswith("_계_총인구수")), None)
    dong_df = dong_df[dong_df[total_pop_col] > 0]

    dong_df["청년인구수"] = dong_df[youth_col_names].sum(axis=1)
    dong_df["청년비율(%)"] = (dong_df["청년인구수"] / dong_df[total_pop_col] * 100).round(2)

    sort_col = "청년인구수" if youth_metric == "청년 인구수" else "청년비율(%)"
    ranked = dong_df.sort_values(sort_col, ascending=False).head(top_n)

    # ---- Plotly 가로 막대 그래프 (Top N) ----
    fig_youth = go.Figure()
    fig_youth.add_trace(
        go.Bar(
            x=ranked[sort_col][::-1],
    ...
    # ---- 요약 테이블 ----
    ranked_display = ranked[["지역명", total_pop_col, "청년인구수", "청년비율(%)"]].rename(
        columns={total_pop_col: "총인구수"}
    )
    ranked_display.insert(0, "순위", range(1, len(ranked_display) + 1))
    ranked_display["총인구수"] = ranked_display["총인구수"].astype(int)
    ranked_display["청년인구수"] = ranked_display["청년인구수"].astype(int)
